/* CONTRAST ON THE PARTNER SCREEN, EVERY ACCENT AND GROUND.
   ...
   The head-to-head bar and the today card are surfaces the general sweep never
   reaches, because it has no partner to open. Both states of the today card —
   a session logged and nothing yet — across both themes, all six accents and
   all four grounds. */
...
const QUIET = [{ ...FEED[0], trained_today: false, last_trained: '2026-09-04', cardio_summary: null }];
...
for (const { accent, theme, ground } of PASSES) {
  for (const state of ['trained', 'quiet']) {
    const page = await browser.newPage({ viewport: { width: 390, height: 1400 } });
    await page.route('**/*', route => {
      const url = route.request().url();
      if (url.startsWith(BASE)) return route.continue();
      const json = body => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(body) });
      if (url.includes('/rpc/forge_partner_feed')) return json(state === 'trained' ? FEED : QUIET);
    ...
    for (const hit of await page.evaluate(AUDIT)) {
      ...
      if (!prev || hit.r < prev.r) seen.set(key, { ...hit, where: `${ground}/${accent}/${theme}/${state}` });
    }
    checked++;
    await page.close();
  }
