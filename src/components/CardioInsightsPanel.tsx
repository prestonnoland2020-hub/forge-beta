...
export function CardioInsightsPanel({range,rangeLabel,frequencyOnly=false,hideFrequency=false}:{range:TimeRange;rangeLabel:string;frequencyOnly?:boolean;hideFrequency?:boolean}){
  ...
  const loggedSessions=records.flatMap(record=>(record.cardioSessions||[]).map(draft=>{const totals=summarizeCardioDraft(draft);return{date:record.date,activity:draft.structure==='circuit'?'Circuit':draft.activity,minutes:totals.minutes,recoveryMinutes:totals.recoveryMinutes,miles:draft.activity==='Run'?totals.distance:undefined,detail:draft.summary}}));
  ...
  const runMiles=visible.filter(session=>session.activity==='Run').reduce((sum,session)=>sum+(session.miles??0),0);
