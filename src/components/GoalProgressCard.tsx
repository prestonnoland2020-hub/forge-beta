...
export function GoalProgressCard({ goal, roadmap }: { goal: CreatedGoal; roadmap: GoalRoadmap }) {
  ...
  return <article className={`goal-progress-card goal-tracker-simple${goal.type==='Endurance'?' endurance-goal':''}${goalReached?' goal-complete':''}`}>
    ...
    {goal.type!=='Endurance'&&<section className="goal-trajectory-chart"><header><div><span>{goalReached?'GOAL COMPLETED':'TRAJECTORY TO GOAL'}</span><strong>{goalReached?'Target reached':'Required path vs. your progress'}</strong></div><b className={trajectoryStatus==='Goal reached'||trajectoryStatus==='On track'||trajectoryStatus==='Progressing'?'on-track':trajectoryStatus==='More progress needed'?'behind':''}>{trajectoryStatus}</b></header><div className="trajectory-plot" ref={trajectoryRef}><svg viewBox={`0 0 ${TW} ${TH}`} width={TW} height={TH} role="img" aria-label={`Goal trajectory: ${trajectoryStatus}`}><g className="trajectory-grid"><line x1={TL} y1="50" x2={TR} y2="50"/><line x1={TL} y1="120" x2={TR} y2="120"/><line x1={TL} y1="190" x2={TR} y2="190"/></g><line className="trajectory-required" x1={TL} y1={chartY(requiredStart)} x2={TR} y2={chartY(target)}/>{datedTrajectory.length>0&&<polyline className="trajectory-calculated" points={evidencePoints}/>} {datedDemonstrated.length>0&&<polyline className="trajectory-demonstrated" points={demonstratedPoints}/>} {predictedAtDeadline&&!goalReached&&datedTrajectory.length>0&&<line className="trajectory-forecast" x1={chartX(datedTrajectory.at(-1)!.date)} y1={chartY(datedTrajectory.at(-1)!.value)} x2={TR} y2={chartY(predictedAtDeadline)}/>} {datedTrajectory.map((item,index)=><circle className="trajectory-point calculated" cx={chartX(item.date)} cy={chartY(item.value)} r="5" key={`calculated-${item.date}-${index}`}><title>{formatDate(item.date)} · {formatValue(item.value)} · {item.label}</title></circle>)}{datedDemonstrated.map((item,index)=><rect className="trajectory-point demonstrated" x={chartX(item.date)-5} y={chartY(item.value)-5} width="10" height="10" key={`actual-${item.date}-${index}`}><title>{formatDate(item.date)} · {formatValue(item.value)} · {item.label}</title></rect>)}<circle className="trajectory-target" cx={TR} cy={chartY(target)} r="6"/><text x={TL} y="218">{formatDate(chartStart)}</text><text x={TR} y="218" textAnchor="end">GOAL · {formatDate(goal.date)}</text><text className="trajectory-target-label" x={TR-10} y={Math.max(18,chartY(target)-10)} textAnchor="end">{goal.target}</text></svg></div><footer><span><i className="required"/>Required</span><span><i/>Best set / day</span><span><i className="demonstrated"/>1RM result</span>{!goalReached&&<span><i className="forecast"/>Forecast</span>}{goalReached?<strong>Reached {currentEvidence?.date?formatDate(currentEvidence.date):'from your saved baseline'}</strong>:strengthForecast?<strong>Deadline range: {formatValue(strengthForecast.low)}–{formatValue(strengthForecast.high)} · {strengthForecast.confidence.toLowerCase()} confidence</strong>:predictedAtDeadline&&<strong>Estimated at deadline: {formatValue(predictedAtDeadline)}</strong>}</footer>{strengthForecast&&<p className="goal-forecast-note">{strengthForecast.reason}</p>}</section>}
