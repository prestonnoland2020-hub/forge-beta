...
export function TopSetHistory({lift,unit='lb',suggestedExercises,onChoose}:Props){
  ...
  return <div className="top-set-history"><div className="history-heading"><span className="field-caption">{lift?`RECENT ${lift.toUpperCase()} SETS`:suggestedExercises?.length?'EXERCISES FROM THIS WORKOUT':'RECENT EXERCISES'}</span><small>{lift?'Tap a previous result to prefill it.':'Choose an exercise to view its history.'}</small></div><div className="recent-set-row contextual">{(lift?selectedHistory.map(set=>({exercise:lift,set})):recentExercises.slice(0,6)).map(({exercise,set})=><button key={`${exercise}-${set.date}-${set.reps}`} type="button" onClick={()=>onChoose(exercise,set)}><span>{lift?new Date(`${set.date}T12:00:00`).toLocaleDateString('en-US',{month:'short',day:'numeric'}):exercise}</span><b>{set.weight}×{set.reps}</b>{lift&&<small>Calc. max {calculateEstimatedOneRepMax(set.weight,set.reps)} {unit}</small>}</button>)}</div>{lift&&!selectedHistory.length&&<p className="empty-history">No previous sets for this exercise.</p>}</div>;
}
