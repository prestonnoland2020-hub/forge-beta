...
export function AdaptiveTrainingProvider({children}:{children:ReactNode}){
  ...
  const loggedRunHistory=useMemo<RunResult[]>(()=>records.flatMap(record=>(record.cardioSessions||[]).filter(session=>session.activity==='Run'&&(session.structure==='steady'||session.structure==='custom')).map(session=>{const totals=summarizeCardioDraft(session);const distanceMiles=cardioMiles(session);return{id:`workout-${record.id}-${session.id}`,date:record.date,kind:'Easy' as const,distanceMiles,durationMinutes:totals.minutes,completed:distanceMiles>0&&totals.minutes>0}})).filter(run=>run.completed),[records]);
  ...
  const profile=useMemo<AdaptiveProfile>(()=>({...prefs,readiness:recovery.confidence==='Low'?100:recovery.readiness,sleepHours:recovery.confidence==='Low'?8:latest.sleepMinutes/60,strengthFatigue:recovery.strengthFatigue,watchConnected:latest.provider!=='Unavailable'}),[prefs,recovery,latest]);
