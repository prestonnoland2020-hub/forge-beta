...
import { qualitySession, qualityPhaseFor } from '../../lib/qualitySession';
...
export function resolveWeekRunning<T extends AiPlanWeek>(
  ...
  const hasQuality = present(week.qualityDay) && (goalPace > 0 || (Boolean(week.quality) && !/no goal/i.test(week.quality)));
  ...
  const written = goalPace && hasQuality
    ? qualitySession({
  ...
  const qualityMiles = hasQuality && !cooked ? (written ? written.miles : qualitySessionMiles(String(week.quality), pace)) : 0;
  ...
  return { ...week, quality: hasQuality ? qualityText : week.quality, qualityPace: written ? '' : week.qualityPace, mileage: scheduled, longRunMiles: longRunMiles || week.longRunMiles, easyDays, easyRuns, easyMinutes };
}
