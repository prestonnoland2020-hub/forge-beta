...
export function buildLongRangePlan(goals:CreatedGoal[],profile:AdaptiveProfile,weeksRequested=16,records:PlanHistoryRecord[]=[]):PlanWeek[]{
  ...
  return Array.from({length:total},(_,index)=>{
    ...
    const quality=!endurance?'No goal-driven cardio':!hasRunBaseline?'Establish a comfortable running baseline':taper?'4–6 relaxed strides with full recovery':test?`Goal effort assessment for ${endurance.exercise||'priority event'}`:phase==='Foundation'?`${6+Math.floor(index/2)} × ${repDistance} m controlled`:phase==='Build'?`${5+Math.floor(index/3)} × ${repDistance} m · threshold to goal effort`:deload?'Short fartlek · stop while fresh':`${4+Math.floor(index/4)} × ${repDistance} m · event-specific pace`;/* The long run is a SHARE of the week (~35%), capped by durability that grows
    ...
    const longMiles=hasRunBaseline?(Math.min(Math.max(3,mileage*.35),durabilityCap)*(taper?.72:test?.55:deload?.85:1)).toFixed(1):'0';const easyMinutes=hasRunBaseline?Math.round(Math.max(20,Math.min(70,mileage/Math.max(1,profile.runningDays)*10))):0;
