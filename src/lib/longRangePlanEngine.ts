...
const eventMiles=(goal?:CreatedGoal)=>{const name=(goal?.exercise||goal?.title||'').toLowerCase();if(name.includes('marathon')&&!name.includes('half'))return 26.219;if(name.includes('half'))return 13.109;if(name.includes('10k'))return 6.214;if(name.includes('5k'))return 3.107;if(/\bmile\b/.test(name))return 1;return 3.107};
const clockText=(seconds:number)=>{const whole=Math.max(1,Math.round(seconds));const minutes=Math.floor(whole/60),rest=whole%60;return `${minutes}:${String(rest).padStart(2,'0')}`};
...
export function buildLongRangePlan(goals:CreatedGoal[],profile:AdaptiveProfile,weeksRequested=16,records:PlanHistoryRecord[]=[]):PlanWeek[]{
  ...
  return Array.from({length:total},(_,index)=>{
    ...
    const easyPaceText=goalPacePerMile?` @ ${clockText(goalPacePerMile*1.28)}–${clockText(goalPacePerMile*1.42)}/mi`:'';
