...
export function GoalsPage({embedded=false}:{embedded?:boolean}={}){
  const {goals,saveGoal,deleteGoal,syncError}=useGoals();const roadmaps=useMemo(()=>buildGoalRoadmaps(goals),[goals]);const [open,setOpen]=useState(false);const [editing,setEditing]=useState<number|null>(null);/* Every goal's full read-out — the four stat tiles, the derivation, the Forge
