...
export function HistoryPage({embedded=false}:{embedded?:boolean}={}){
  ...
  const [selectedId,setSelectedId]=useState(trainingRecords[0]?.id??'');const [selectedDate,setSelectedDate]=useState(trainingRecords[0]?.date??iso(now.getFullYear(),now.getMonth(),now.getDate()));/* THE MONTH ON SCREEN AND THE DAY UNDER IT HAVE TO AGREE. The calendar
     opened on the current month while the card beneath it read out the most
     recent training day — so an athlete who last trained in August landed on
     an empty September grid, 0.0 miles, 0 lifts, above a card describing
     Aug 23. The selection is the one source: the grid opens on its month. */
  const [cursor,setCursor]=useState(()=>{const anchor=trainingRecords[0]?.date;return anchor?new Date(Number(anchor.slice(0,4)),Number(anchor.slice(5,7))-1,1):new Date(now.getFullYear(),now.getMonth(),1)});const selected=records.find(record=>record.id===selectedId&&record.date===selectedDate);const year=cursor.getFullYear(),month=cursor.getMonth(),days=new Date(year,month+1,0).getDate(),leading=(new Date(year,month,1).getDay()+6)%7;
  ...
  const selectDate=(date:string)=>{setSelectedDate(date);const record=trainingRecords.find(item=>item.date===date);setSelectedId(record?.id||'')};const selectedHasTraining=selected?hasTraining(selected):false;const shiftMonth=(change:number)=>setCursor(new Date(year,month+change,1));

  return <div className="stack-xl history-page">
    {/* THE SAME INSTRUCTION, TWICE. "Tap a day to see it — or to add and edit
        what you did" sat above the calendar, and "Tap a day · swipe to change
        month" sat inside it. One of them had to go, and the one that goes is
        the one further from the thing it describes. */}
    {records.length?<>
      <section className="card history-calendar restored-calendar classic-calendar" onTouchStart={event=>{calendarTouchStart.current=event.touches[0]?.clientX??null}} onTouchEnd={event=>{if(calendarTouchStart.current===null)return;const distance=(event.changedTouches[0]?.clientX??calendarTouchStart.current)-calendarTouchStart.current;calendarTouchStart.current=null;if(Math.abs(distance)>55)shiftMonth(distance<0?1:-1)}}><div className="history-toolbar"><button onClick={()=>shiftMonth(-1)} aria-label="Previous month">‹</button><div><h3>{months[month]} {year}</h3><small>Tap a day · swipe to change month</small></div><button onClick={()=>shiftMonth(1)} aria-label="Next month">›</button></div><div className="history-summary calendar-kpis"><div><strong>{monthMiles.toFixed(1)}</strong><span>Miles</span></div><div><strong>{monthStrengthDays}</strong><span>Lifts</span></div><div><strong>{monthCardioDays}</strong><span>Cardio</span></div><div><strong>{monthAverageWeight===null?'—':monthAverageWeight.toFixed(1)}</strong><span>Avg {weightUnit}</span></div></div><div className="weekday-row">{['MO','TU','WE','TH','FR','SA','SU'].map(day=><span key={day}>{day}</span>)}</div><div className="calendar-grid">{Array.from({length:leading}).map((_,index)=><i key={`empty-${index}`}/>)}{Array.from({length:days},(_,index)=>index+1).map(day=>{const date=iso(year,month,day);const activity=activeDateKinds.get(date);const trained=Boolean(activity);const isToday=date===todayIso;return <button key={date} className={`${trained?'trained ':''}${isToday?'is-today ':''}${selectedDate===date?'selected':''}`} onClick={()=>selectDate(date)} aria-label={`${formatDate(date)}${isToday?' — today':''}${trained?' — preview training day':' — preview empty day'}`} aria-current={isToday?'date':undefined}><span>{day}</span>{activity&&<span className="calendar-markers">{activity.strength&&<i className="strength"/>}{activity.cardio&&<i className="cardio"/>}</span>}</button>})}</div><div className="calendar-legend"><span><i className="strength"/>Lift</span><span><i className="cardio"/>Cardio</span></div></section>
      <section className="card simple-day-preview compact-day-preview"><header><div><span className="eyebrow">SELECTED DAY</span><h2>{formatDate(selectedDate)}</h2></div><Link className="button" to={selectedHasTraining&&selected?`/workout?edit=${selected.id}`:`/workout?date=${selectedDate}`}>{selectedHasTraining?'Edit day':'Add training'}</Link></header>{selectedHasTraining&&selected?<><p>{strengthMuscles(selected).join(' · ')||(hasCardio(selected)?'Cardio day':'')}</p>{/* "Completed from Forge recommendation · split position 7" is how the
                record is stored, not what happened. What happened is that Forge
                asked for this day and the athlete did it. The position is a
                cursor into the split — useful in a bug report, meaningless on a
                Tuesday. */}
            {selected.recommendationId&&<small className="recommendation-lineage">Forge planned this day</small>}<div className="simple-preview-results">{recordTopSets(selected).map((set,index)=><div key={`${set.lift}-${index}`}><strong>{set.lift}</strong><span>{set.weight} {weightUnit} ×{set.reps}</span></div>)}{recordTopSets(selected).length===0&&strengthMuscles(selected).length>0&&<div><strong>Strength workout</strong><span>{strengthMuscles(selected).join(' · ')} · no historical top set recorded</span></div>}{(selected.cardioSessions||[]).map(session=><div key={session.id}><strong>{session.activity}</strong><span>{formatCardioSummary(session)}</span></div>)}</div>{selected.notes&&<aside className="selected-day-notes"><span className="eyebrow">NOTES</span><p>{selected.notes}</p></aside>}</>:<p>No training is logged for this date.</p>}</section>
