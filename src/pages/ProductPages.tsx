...
function WorkoutEditor() {
  ...
  const restrictToPlannedExercises=usingSplit&&plannedExerciseNames.size>0&&!showAllExercises;
  ...
  const allowedStrengthExercises=strengthCatalogue.filter(exercise=>restrictToPlannedExercises
    ...
    :(!usingSplit||!plannedExerciseNames.size)&&(!usingSplit||!sourceMuscles.length||exercise.muscles.some(muscle=>sourceMuscles.includes(muscle)))||showAllExercises||plannedExerciseNames.has(exercise.name.trim().toLowerCase()));
  ...
  const cardioOnlyDay=usingSplit&&!editingRecord&&recommendation?.splitDay.type==='cardio';const manualLiftData=exercises.find(exercise=>exercise.name===lift);const manualTopSet:LoggedTopSet|undefined=lift&&Number(weight)&&Number(reps)?{muscle:manualLiftData?.muscles.find(muscle=>sourceMuscles.includes(muscle))||manualLiftData?.muscles[0]||sourceMuscles[0]||'Primary',lift,weight:Number(weight),reps:Number(reps),completed:true}:undefined;
  ...
  const saveWorkout=async()=>{
    ...
    const planDayActive=(usingSplit||dayOverride)&&(plannedDay?.muscles||[]).some(muscle=>muscle!=='Cardio');if(!editingRecord&&!completedTopSets.length&&!lift&&!hasCardio&&!quickLoggedKeys.length&&!bodyWeight&&!notes.trim()&&!effort&&!planDayActive){setSaveMessage('Add a top set or cardio entry before saving.');return}
