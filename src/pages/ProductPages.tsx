...
function WorkoutEditor() {
  ...
  return <div className="narrow stack-xl workout-editor">{/* THE INTRO DESCRIBED THE BUTTONS DIRECTLY UNDER IT. "Start from today's
    ...
    <CardioBuilder onEntriesChange={(hasEntries,entries)=>{setHasCardio(hasEntries);setCardioSessions(entries);persistCardio(entries)}} initialOpen={Boolean(searchParams.get('cardio'))} initialEntries={editingRecord?.cardioSessions??todayRecord?.cardioSessions} plannedSummary={workoutSource==='plan'&&recommendation?.cardio?.selected?recommendation.cardio.summary:undefined} plannedCircuit={workoutSource==='plan'&&recommendation?.cardio?.selected&&recommendation.cardio.session.plan.structure==='Circuit'?{name:recommendation.cardio.title,plan:recommendation.cardio.session.plan}:undefined}/>
