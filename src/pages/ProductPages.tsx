...
import { ForgeWritesIt, type WrittenSession } from '../components/ForgeWritesIt';
...
function WorkoutEditor() {
  ...
  /* A session Forge wrote for a day outside the split — see ForgeWritesIt. */
  const [written,setWritten]=useState<WrittenSession|null>(null);
  ...
  /* CHOOSING A SPLIT DAY GETS THE REAL PRESCRIPTION, NOT A BLANK ONE.
     ...
     to be empty. */
  /* SWITCHING TO A SPLIT DAY PRESCRIBES THAT DAY. Every one of these handlers
     used to call setTopSets([]) — so choosing "Chest & Back 2" out of the split
     emptied the section instead of filling it, and the athlete was left with a
     named day, its muscles printed underneath, and nothing to log against it.
     The prescription for a chosen day already existed (splitDayTopSets, built
     from the same engine call Today makes); nothing was wired to it. */
  /* THE CHOSEN DAY'S OWN PRESCRIPTION, cardio included. A HYROX day picked
     from the split used to show "hyrox" and an empty cardio card, because
     only the due day's recommendation reached the page. */
  const chosenRecommendation=useMemo(()=>workoutSource==='split'&&dayOverride&&savedDay?recommendationFor(selectedPlanDay+1,savedDay.name):recommendation,[workoutSource,dayOverride,savedDay,selectedPlanDay,recommendationFor,recommendation]);
  const prescribeSplitDay=(index:number):LoggedTopSet[]=>{
    ...
    /* No prescription for this day yet — a lift with no logged history has
       ...
       to log, which is what makes a first session possible at all. */
    return (day?.exercises||[]).flatMap(name=>{
      const exercise=exercises.find(item=>item.name===name);
      if(!exercise?.enabled||!isProgrammableStrength(exercise))return[];
      const muscles=exercise.muscles.filter(muscle=>muscle!=='Cardio');
      return[{muscle:muscles.find(muscle=>(day?.muscles||[]).includes(muscle))||muscles[0]||'Primary',lift:exercise.name,weight:0,reps:0,completed:true}];
    });
  };
  /* AND WHAT IS ALREADY LOGGED TODAY SURVIVES THE SWITCH. Changing source is
     changing what is PLANNED; it is not undoing work already saved. */
  const keepingTodaysWork=(planned:LoggedTopSet[]):LoggedTopSet[]=>{
    const saved=(records.find(record=>record.date===sessionIso)?.topSets||[]).filter(set=>set.completed!==false);
    const done=new Set(saved.map(set=>canonicalLiftKey(set.lift)));
    return [...saved,...planned.filter(set=>!done.has(canonicalLiftKey(set.lift)))];
  };
  /* ONE BUILDER. This was a second copy of the same mapping, which is how the
  ...
  /* THE RECORD KEEPS WHAT A CARDIO MOVEMENT USES. Rowing's library entry lists
     Back, Quads, Hamstrings and Glutes, and that is true — it is how Forge
     knows a hard row leaves the back tired, and recovery and fatigue are
     entitled to it. What it must never do is count as a BACK DAY, and that is a
     question about reading, not about writing: the frequency chart excludes
     anything a cardio movement put there. Nothing is dropped at the point of
     saving. */
  ...
  /* CARDIO-ONLY IS A PROPERTY OF THE DAY BEING LOGGED, not of whatever Forge
     happened to recommend this morning. This read recommendation.splitDay, so
     on a day Forge called a long run the whole top-set section disappeared —
     and picking a lifting day out of the split did not bring it back. The log
     went 01 CARDIO, 03 SESSION DETAILS, with no way to record a single lift. */
  const loggedDayType=String(plannedDay?.type||'').toLowerCase();
  const cardioOnlyDay=usingSplit&&!editingRecord&&(loggedDayType==='cardio'||loggedDayType==='hyrox');
  ...
  const dayLabel=usingSplit?(plannedDay?.name||''):written?.name||freeMuscles.filter(muscle=>muscle!=='Cardio').join(' + ');
  ...
  const persistCardio=(allEntries:CardioLogDraft[])=>{
    ...
    addRecord({date:sessionIso,title:usingSplit?(plannedDay?.name||'Planned Workout'):written?.name||'Custom Workout',
  ...
  const choosePlanDay=(index:number)=>{setSelectedPlanDay(index);setTopSets(keepingTodaysWork(prescribeSplitDay(index)))};
  const saveWorkout=async()=>{
    ...
    const title=assignedName||editingRecord?.title||existingDay?.title||(usingSplit?(plannedDay?.name||'Planned Workout'):written?.name||'Custom Workout');
    ...
    let result=editingRecord?updateRecord(editingRecord.id,draft):addRecord(draft);
    /* FINISHING A DAY WHOSE SETS WERE ALREADY SAVED IS NOT A DUPLICATE. Every
       set is saved the moment it is logged, so by Finish Day the record
       already holds them — and addRecord, seeing nothing new, refused with
       "already in your history" and the button did nothing. The day's details
       (effort, notes, body weight) go onto the existing record instead. */
    if(!result.ok&&'duplicate' in result&&result.duplicate&&!editingRecord){
      const day=result.duplicate;
      result=updateRecord(day.id,{...day,effort:effort||day.effort,notes:notes.trim()||day.notes,bodyWeight:bodyWeightToSave||day.bodyWeight,muscles:draft.muscles.length?Array.from(new Set([...day.muscles,...draft.muscles])):day.muscles});
    }
  ...
  return <div className="narrow stack-xl workout-editor">{/* THE INTRO DESCRIBED THE BUTTONS DIRECTLY UNDER IT. "Start from today's
        plan, pick a day from your split, or record only what you did" is the
        three source buttons, read out. Editing and back-dating still get a
        line, because those two states are not obvious from the screen. */}
    <PageIntro copy={editingRecord?'Update the saved results for this day.':requestedDate?'Log anything you completed on this date — it saves as one training day.':''} />
    ...
    {!editingRecord&&<section className="card workout-source-card compact-workout-source"><div className="workout-date"><span className="eyebrow">LOGGING · {todayShort.toUpperCase()}</span><strong>{workoutSource==='plan'?(plannedDay?.name||'Today’s recommendation'):workoutSource==='split'?(savedDay?.name||'Choose a split day'):written?.name||'Start fresh'}</strong></div><div className="source-options"><button className={workoutSource==='plan'?'active':''} onClick={()=>{setWorkoutSource('plan');setDayOverride(false);setSelectedPlanDay(recommendationIndex);setTopSets(keepingTodaysWork(recommendationTopSets))}}><span>FORGE</span><b>Recommended</b></button><button className={workoutSource==='split'?'active':''} onClick={()=>{setWorkoutSource('split');setDayOverride(true);setTopSets(keepingTodaysWork(prescribeSplitDay(selectedPlanDay)))}}><span>SPLIT</span><b>Choose day</b></button><button className={workoutSource==='blank'?'active':''} onClick={()=>{setWorkoutSource('blank');setDayOverride(false);setTopSets([])}}><span>FRESH</span><b>Start fresh</b></button></div>{workoutSource==='split'&&(savedDays.length>0?<label className="plan-day-picker">Which day of your split<select value={selectedPlanDay} onChange={event=>{const index=Number(event.target.value);setSelectedPlanDay(index);setDayOverride(true);setTopSets(keepingTodaysWork(prescribeSplitDay(index)))}}>{savedDays.map((day,index)=><option key={`${day.name}-${index}`} value={index}>{day.name}{recommendation&&index===recommendationIndex?' · due today':''}</option>)}</select></label>:<p className="empty-history">Build a split in Plan first — then its days appear here.</p>)}{workoutSource==='split'&&savedDay&&<div className="saved-recommendation-source"><span>Split position {selectedPlanDay+1}</span><strong>{(savedDay.muscles||[]).filter(muscle=>muscle!=='Cardio').join(' · ')||(savedDay.dayType==='hyrox'?'HYROX — Forge wrote the session':savedDay.dayType?savedDay.dayType[0].toUpperCase()+savedDay.dayType.slice(1):'Training day')}</strong><small>Logged against this day, wherever the cycle is.</small></div>}{workoutSource==='plan'&&recommendation&&<div className="saved-recommendation-source"><span>Split position {recommendation.splitDay.position}</span><strong>{recommendation.splitDay.muscles.join(' · ')||(recommendation.splitDay.type==='hyrox'?'HYROX — Forge wrote the session':recommendation.splitDay.type[0].toUpperCase()+recommendation.splitDay.type.slice(1))}</strong><small>Synced with Today and Coach.</small></div>}{workoutSource==='blank'&&!written&&<div className="free-muscle-picker"><div><span className="field-caption">WHAT DID YOU TRAIN?</span><small>Pick the muscle groups — they decide which exercises are offered and what this day counts toward.</small></div><div className="muscle-picker">{muscleOptions.filter(muscle=>muscle!=='Cardio').map(muscle=><button type="button" className={freeMuscles.includes(muscle)?'muscle-chip active':'muscle-chip'} onClick={()=>toggleFreeMuscle(muscle)} key={muscle}>{muscle}</button>)}</div>{!freeMuscles.length&&<small className="free-muscle-hint">Cardio-only day? Leave these empty and log it below.</small>}</div>}</section>}
    ...
    {!editingRecord&&workoutSource==='blank'&&<ForgeWritesIt dateIso={sessionIso} session={written} onSession={setWritten}/>}
    {!topSets.length&&!cardioOnlyDay&&<section className="card form-card top-set-launch"><div className="section-title compact-title"><div><h3>Top sets</h3><p>The one heaviest meaningful set per lift.</p></div></div><button type="button" className="button add-top-set-button" onClick={openTopSetSheet}>＋ Log a top set</button></section>}
    ...
    <CardioBuilder onEntriesChange={(hasEntries,entries)=>{setHasCardio(hasEntries);setCardioSessions(entries);persistCardio(entries)}} initialOpen={Boolean(searchParams.get('cardio'))} initialEntries={editingRecord?.cardioSessions??todayRecord?.cardioSessions} plannedSummary={written&&workoutSource==='blank'?written.summary:usingSplit&&chosenRecommendation?.cardio?.selected?chosenRecommendation.cardio.summary:undefined} plannedCircuit={written&&workoutSource==='blank'?{name:written.name,plan:written.plan}:usingSplit&&chosenRecommendation?.cardio?.selected&&chosenRecommendation.cardio.session.plan.structure==='Circuit'?{name:chosenRecommendation.cardio.title,plan:chosenRecommendation.cardio.session.plan}:undefined}/>
