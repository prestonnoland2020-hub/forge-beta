...
function WorkoutEditor() {
  ...
  const completedOnEntry=!editId&&!requestedDate&&recommendation?.status==='completed'?records.find(record=>record.date===todayIso):undefined;
  ...
  const topSetExerciseOptions=strengthCatalogue.filter(exercise=>!newTopSetMuscle||exercise.muscles.includes(newTopSetMuscle));
  const cardioOnlyDay=workoutSource==='plan'&&!editingRecord&&recommendation?.splitDay.type==='cardio';const manualLiftData=exercises.find(exercise=>exercise.name===lift);const manualTopSet:LoggedTopSet|undefined=lift&&Number(weight)&&Number(reps)?{muscle:manualLiftData?.muscles.find(muscle=>sourceMuscles.includes(muscle))||manualLiftData?.muscles[0]||sourceMuscles[0]||'Primary',lift,weight:Number(weight),reps:Number(reps),completed:true}:undefined;
  const calculatedMax=calculateEstimatedOneRepMax(Number(weight),Number(reps));
  const previousRecord=records.filter(record=>sameLift(record.lift||'',lift)&&record.weight&&record.reps).sort((a,b)=>b.date.localeCompare(a.date))[0];
  ...
  const chooseRepeatedWorkout=(id:string)=>{setSelectedRecent(id);setTopSets(repeatTopSetsFor(id));setLift('');setWeight('');setReps('1');setNewTopSetMuscle('')};
  ...
  /* Exercise names must stay comparable across users. An exact normalized
     match silently selects the existing library exercise; a near match asks
     once before creating a duplicate spelling. Canonical alias curation is
     the long-term answer at scale — this stops the everyday dupes. */
  const normalizeExerciseName=(value:string)=>value.toLowerCase().replace(/[^a-z0-9]+/g,' ').replace(/\b(barbell|bb)\b/g,'barbell').replace(/s\b/g,'').trim();
  const findExerciseMatch=(name:string)=>{const target=normalizeExerciseName(name);if(!target)return null;let near:string|null=null;for(const exercise of exercises){const existing=normalizeExerciseName(exercise.name);if(existing===target)return{kind:'exact' as const,name:exercise.name};if(!near&&(existing.includes(target)||target.includes(existing)))near=exercise.name}return near?{kind:'near' as const,name:near}:null};
  const [dupeWarning,setDupeWarning]=useState('');
  const saveNewExercise=()=>{const name=newExerciseName.trim();if(!name||!newExerciseMuscles.length)return;const match=findExerciseMatch(name);if(match&&match.kind==='exact'){setLift(match.name);setHistoryExercise(match.name);setAddingExercise(false);setNewExerciseName('');setNewExerciseMuscles([]);setDupeWarning(`Matched to “${match.name}” — it already exists, so results stay comparable.`);return}if(match&&match.kind==='near'&&dupeWarning!==`__confirm:${name}`){setDupeWarning(`__confirm:${name}`);return}setDupeWarning('');const created=addExercise({name,kind:'Strength',muscles:newExerciseMuscles,detail:'Added while logging · Weight + reps',enabled:true,custom:true});setLift(created.name);setHistoryExercise(created.name);setAddingExercise(false);setNewExerciseName('');setNewExerciseMuscles([])};
  ...
  const recommendationMetadata=editingRecord
    ...
    :workoutSource==='plan'?(dayOverride?{splitId:activeSplitId,splitPosition:selectedPlanDay+1}:recommendation?{recommendationId:recommendation.id,splitId:recommendation.splitDay.splitId,splitDayId:recommendation.splitDay.id,splitPosition:recommendation.splitDay.position}:{}):{};
  ...
  const persistCardio=(entries:CardioLogDraft[])=>{
    ...
    addRecord({date:sessionIso,title:workoutSource==='repeat'?repeatedWorkout.name:workoutSource==='plan'?(plannedDay?.name||'Planned Workout'):'Custom Workout',
      muscles:['Cardio'],cardioSessions:entries,hasCardio:true,...recommendationMetadata});
  };
  const quickLogTopSet=(set:LoggedTopSet)=>{if(!set.lift||!set.weight||!set.reps)return;const completed={...set,completed:true,calculatedMax:calculateEstimatedOneRepMax(set.weight,set.reps)??undefined};const primary=liftPrimary(set.lift);const result=addRecord({date:sessionIso,title:`Top set · ${set.lift}`,muscles:primary.length?primary:[set.muscle],topSets:[completed],lift:set.lift,weight:set.weight,reps:set.reps,calculatedMax:completed.calculatedMax,hasCardio:false,...recommendationMetadata,selectedRecommendationTopSetIds:set.recommendationTopSetId?[set.recommendationTopSetId]:[]});if(!result.ok){setQuickLogMessage('That exact top set is already saved.');return}
    ...
    setQuickLoggedKeys(current=>[...current,topSetKey(set)]);setQuickLogMessage(`${set.lift} · ${set.weight} ${weightUnit} ×${set.reps} saved — keep logging, then Finish Day when you’re done.`);if(!topSets.length){setLift('');setWeight('');setReps('1')}};
  ...
  const saveWorkout=async()=>{
    ...
    const planDayActive=(workoutSource==='plan'||dayOverride)&&(plannedDay?.muscles||[]).some(muscle=>muscle!=='Cardio');if(!editingRecord&&!completedTopSets.length&&!lift&&!hasCardio&&!quickLoggedKeys.length&&!bodyWeight&&!notes.trim()&&!effort&&!planDayActive){setSaveMessage('Add a top set or cardio entry before saving.');return}
    ...
    const title=editingRecord?.title||existingDay?.title||(workoutSource==='repeat'?repeatedWorkout.name:workoutSource==='plan'?(plannedDay?.name||'Planned Workout'):'Custom Workout');
    ...
    const assignedDayMuscles=(editingRecord
      ...
      :((workoutSource==='plan'||dayOverride)?plannedDay?.muscles:undefined))?.filter(muscle=>muscle!=='Cardio');
  ...
  return <div className="narrow stack-xl workout-editor"><PageIntro eyebrow={todayLong.toUpperCase()} title={editingRecord?'Edit training day':requestedDate?'Add a training day':"Log today's workout"} copy={editingRecord?'Update the saved results for this day. History and insights will refresh automatically.':requestedDate?'Log anything you completed on this date. It will be saved as one editable training day.':'Start with the plan, repeat a previous session, or record only what you performed.'} />
    ...
    {!editingRecord&&<section className="card workout-source-card compact-workout-source"><div className="workout-date"><span className="eyebrow">LOGGING · {todayShort.toUpperCase()}</span><strong>{workoutSource==='plan'?(plannedDay?.name||'Today’s recommendation'):workoutSource==='repeat'?'Repeat workout':'Start blank'}</strong></div><div className="source-options"><button className={workoutSource==='plan'?'active':''} onClick={()=>{setWorkoutSource('plan');setTopSets(recommendationTopSets)}}><span>FORGE</span><b>Today</b></button><button className={workoutSource==='repeat'?'active':''} onClick={()=>{setWorkoutSource('repeat');setTopSets(repeatTopSetsFor(selectedRecent));setLift('');setWeight('');setReps('1')}}><span>REUSE</span><b>Repeat</b></button><button className={workoutSource==='blank'?'active':''} onClick={()=>{setWorkoutSource('blank');setTopSets([]);setLift('');setWeight('');setReps('1')}}><span>FREE</span><b>Blank</b></button></div>{workoutSource==='plan'&&savedDays.length>0&&<label className="plan-day-picker">Split day being logged<select value={selectedPlanDay} onChange={event=>{const index=Number(event.target.value);setSelectedPlanDay(index);setDayOverride(true);setTopSets([])}}>{savedDays.map((day,index)=><option key={`${day.name}-${index}`} value={index}>{day.name}{recommendation&&index===recommendationIndex?' · due today':''}</option>)}</select></label>}{workoutSource==='plan'&&recommendation&&<div className="saved-recommendation-source"><span>Split position {recommendation.splitDay.position}</span><strong>{recommendation.splitDay.muscles.join(' · ')||recommendation.splitDay.type}</strong><small>Synced with Today and Coach.</small></div>}{workoutSource==='repeat'&&<div className="recent-workout-picker"><div><span className="field-caption">CHOOSE A RECENT WORKOUT</span><small>The structure copies over; completed values stay blank.</small></div>{!recentWorkouts.length&&<p className="empty-history">No completed workouts to repeat yet.</p>}{recentWorkouts.map(item=><button className={selectedRecent===item.id?'active':''} onClick={()=>chooseRepeatedWorkout(item.id)} key={item.id}><span>{item.date}</span><div><strong>{item.name}</strong><small>{item.detail}</small></div><b>{selectedRecent===item.id?'Selected':'Choose'}</b></button>)}</div>}</section>}
    {!topSets.length&&!cardioOnlyDay&&<section className="card form-card accent-card"><div className="section-title compact-title"><span>01</span><div><h3>Exercise & Top Set <small>(optional)</small></h3><p>{workoutSource==='plan'&&sourceMuscles.length?`Showing exercises mapped to ${sourceMuscles.join(', ')} from today’s split.`:'Every completed weighted set receives a calculated max.'}</p><small className="topset-philosophy">Forge tracks your top set — the one heaviest meaningful set per lift. Warmups, back-offs, and accessories are yours to train; the top set is what measures progress.</small></div></div>
<label>Lift<select value={lift} onChange={event=>{if(event.target.value==='__new__'){setAddingExercise(true);setNewExerciseMuscles(sourceMuscles.filter(muscle=>muscle!=='Cardio'));return}setLift(event.target.value)}}><option value="">Choose a lift</option>{otherStrengthExercises.length&&allowedStrengthExercises.length?<><optgroup label={plannedDay?.name?`${plannedDay.name} (this split day)`:'This workout'}>{allowedStrengthExercises.map(exercise=><option key={exercise.id}>{exercise.name}</option>)}</optgroup><optgroup label="All exercises">{otherStrengthExercises.map(exercise=><option key={exercise.id}>{exercise.name}</option>)}</optgroup></>:[...allowedStrengthExercises,...otherStrengthExercises].map(exercise=><option key={exercise.id}>{exercise.name}</option>)}<option value="__new__">＋ Add a new exercise…</option></select></label>
{addingExercise&&<div className="inline-exercise-creator"><label>Exercise name<input value={newExerciseName} onChange={event=>setNewExerciseName(event.target.value)} placeholder="e.g. Smith incline bench"/></label><div><span className="field-caption">MUSCLE GROUPS</span><div className="muscle-picker">{muscleOptions.filter(muscle=>muscle!=='Cardio').map(muscle=><button type="button" className={newExerciseMuscles.includes(muscle)?'muscle-chip active':'muscle-chip'} onClick={()=>toggleNewExerciseMuscle(muscle)} key={muscle}>{muscle}</button>)}</div></div><div className="button-row"><button type="button" className="button" disabled={!newExerciseName.trim()||!newExerciseMuscles.length} onClick={saveNewExercise}>Save to library & select</button><button type="button" className="button ghost" onClick={()=>{setAddingExercise(false);setDupeWarning('')}}>Cancel</button></div>{dupeWarning&&<small className="inline-exercise-error">{dupeWarning.startsWith('__confirm:')?`“${dupeWarning.slice(10)}” looks similar to an existing exercise — tap Save again to create it anyway.`:dupeWarning}</small>}</div>}<TopSetHistory lift={lift} unit={weightUnit} suggestedExercises={workoutSource==='repeat'?repeatedWorkout.exercises:workoutSource==='plan'?allowedStrengthExercises.map(exercise=>exercise.name):undefined} onChoose={(name,set)=>useRecent(name,String(set.weight),String(set.reps))} /><div className="field-grid"><label>Weight ({weightUnit})<input value={weight} onChange={e=>setWeight(e.target.value)} inputMode="decimal" placeholder="0" /><div className="step-controls"><button onClick={()=>setWeight(String(Math.max(0,Number(weight||0)-5)))}>−5</button><button onClick={()=>setWeight(String(Number(weight||0)+5))}>+5</button></div></label><label>Reps<input value={reps} onChange={e=>setReps(e.target.value)} inputMode="numeric" /><div className="step-controls"><button onClick={()=>setReps(String(Math.max(1,Number(reps||1)-1)))}>−1</button><button onClick={()=>setReps(String(Number(reps||0)+1))}>+1</button></div></label></div>{weight&&reps&&<div className="calculated-max"><div><span>{Number(reps)===1?'REAL 1RM':'CALCULATED MAX'}</span><strong>{calculatedMax??'—'} <small>{weightUnit}</small></strong></div><div><b>{Number(reps)===1?'Logged one-rep result':'Epley estimate'}</b><small>{Number(reps)===1?'Counts toward Real 1RM goals':`${weight} × (1 + ${reps} ÷ 30)`}</small></div></div>}{lift&&previousSet&&<div className="last-set"><span>LAST TIME ON {lift.toUpperCase()}</span><strong>{previousSet.weight} lb ×{previousSet.reps}</strong><small>Calculated max {previousCalculatedMax} lb</small><b>{weight?`Today: ${weight} ${weightUnit} ×${reps} · Calc. max ${calculatedMax} ${weightUnit}`:'Enter today’s set'}</b></div>}<small className="calculated-max-rule">Only saved completed sets become measurements in Insights.</small></section>}
    ...
    {topSets.length>0&&<details className="card form-card add-top-set-card"><summary><h3>Add another top set</h3><p>Choose a muscle group from {plannedDay?.name||'this workout'} first. Exercise choices stay inside that split-day mapping.</p></summary><div className="field-grid"><label>Muscle group<select value={newTopSetMuscle} onChange={event=>{setNewTopSetMuscle(event.target.value);setLift('')}}><option value="">Choose muscle group</option>{sourceMuscles.filter(muscle=>muscle!=='Cardio').map(muscle=><option key={muscle}>{muscle}</option>)}</select></label><label>Exercise<select value={lift} onChange={event=>setLift(event.target.value)} disabled={!newTopSetMuscle}><option value="">Choose exercise</option>{topSetExerciseOptions.filter(exercise=>!topSets.some(set=>sameLift(set.lift,exercise.name))).map(exercise=><option key={exercise.id}>{exercise.name}</option>)}</select></label><label>Weight ({weightUnit})<input value={weight} onChange={event=>setWeight(event.target.value)} inputMode="decimal"/></label><label>Reps<input value={reps} onChange={event=>setReps(event.target.value)} inputMode="numeric"/></label></div><button type="button" className="button" disabled={!newTopSetMuscle||!lift||!Number(weight)||!Number(reps)} onClick={addAnotherTopSet}>Add completed top set</button></details>}
