...
{/* The sheet is reachable from an empty day too. Without this it appeared
    only once a set already existed, so the athlete whose day started blank —
    exactly the one with nothing to work from — was the one who could not
    reach it. */}
<button type="button" className="button ghost add-top-set-button" onClick={()=>setAddingTopSet(true)}>＋ Add a top set</button>
...
{addingExercise&&<div className="inline-exercise-creator"><label>Exercise name<input value={newExerciseName} onChange={event=>setNewExerciseName(event.target.value)} placeholder="e.g. Smith incline bench"/></label><div><span className="field-caption">MUSCLE GROUPS</span><div className="muscle-picker">{muscleOptions.filter(muscle=>muscle!=='Cardio').map(muscle=><button type="button" className={newExerciseMuscles.includes(muscle)?'muscle-chip active':'muscle-chip'} onClick={()=>toggleNewExerciseMuscle(muscle)} key={muscle}>{muscle}</button>)}</div></div><div className="button-row"><button type="button" className="button" disabled={!newExerciseName.trim()||!newExerciseMuscles.length} onClick={saveNewExercise}>Save to library & select</button><button type="button" className="button ghost" onClick={()=>{setAddingExercise(false);setDupeWarning('')}}>Cancel</button></div>{dupeWarning&&<small className="inline-exercise-error">{dupeWarning.startsWith('__confirm:')?`“${dupeWarning.slice(10)}” looks similar to an existing exercise — tap Save again to create it anyway.`:dupeWarning}</small>}</div>}<TopSetHistory lift={lift} unit={weightUnit} suggestedExercises={usingSplit?allowedStrengthExercises.map(exercise=>exercise.name):undefined} onChoose={(name,set)=>useRecent(name,String(set.weight),String(set.reps))} /><div className="field-grid dial-grid"><DialField label={`Weight (${weightUnit})`} kind="weight" unit={weightUnit} value={weight} onChange={setWeight} /><DialField label="Reps" kind="reps" value={reps} onChange={setReps} /></div>{weight&&reps&&<div className="calculated-max"><div><span>{Number(reps)===1?'REAL 1RM':'CALCULATED MAX'}</span><strong>{calculatedMax??'—'} <small>{weightUnit}</small></strong></div><div><b>{Number(reps)===1?'Logged one-rep result':'Epley estimate'}</b><small>{Number(reps)===1?'Counts toward Real 1RM goals':`${weight} × (1 + ${reps} ÷ 30)`}</small></div></div>}{lift&&previousSet&&<div className="last-set"><span>LAST TIME ON {lift.toUpperCase()}</span><strong>{previousSet.weight} lb ×{previousSet.reps}</strong><small>Calculated max {previousCalculatedMax} lb</small><b>{weight?`Today: ${weight} ${weightUnit} ×${reps} · Calc. max ${calculatedMax} ${weightUnit}`:'Enter today’s set'}</b></div>}<small className="calculated-max-rule">Only saved completed sets become measurements in Insights.</small></section>}
    {addingTopSet&&<TopSetSheet unit={weightUnit} exercises={strengthCatalogue} suggested={allowedStrengthExercises.map(exercise=>exercise.name)} dayMuscles={sourceMuscles.filter(muscle=>muscle!=='Cardio')} existing={topSets.map(set=>set.lift).filter(Boolean)} onClose={()=>setAddingTopSet(false)} onSave={saveTopSetFromSheet}/>}
    {topSets.length>0&&<TopSetCards planLabel={plannedDay?.name?`FROM ${plannedDay.name.toUpperCase()}`:undefined} sets={topSets} onChange={updateTopSet} onQuickLog={quickLogTopSet} onEditLogged={editLoggedTopSet} loggedKeys={loggedTopSetKeys} exercises={allowedStrengthExercises} muscles={sourceMuscles.filter(muscle=>muscle!=='Cardio')} records={records} date={sessionIso} unit={weightUnit} onAdd={()=>setAddingTopSet(true)} onRemove={removeTopSet} onCreateExercise={createExerciseForTopSet}/>}
