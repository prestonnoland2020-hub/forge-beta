...
type View='profile'|'coach'|'connections'|'appearance'|'billing'|'devices'|'faq';
const VIEWS=['profile','coach','connections','appearance','billing','devices','faq'] as const;
...
export function ProfilePage(){
  ...
  return <div className="stack-xl simple-profile">
    ...
    <nav className="profile-tabs">{([['profile','Profile'],['billing','Plan'],['coach','Coach'],['connections','Connections'],['appearance','Appearance']] as const).map(([key,label])=><button className={view===key?'active':''} onClick={()=>setView(key)} key={key}>{label}</button>)}</nav>
    {/* Every subview gets a way back. The tab strip switches, but a back arrow
        is what a phone thumb expects when it has stepped into a setting. */}
    {view!=='profile'&&<button type="button" className="profile-back" onClick={()=>setView('profile')}>‹ Back to profile</button>}
    ...
    {view==='profile'&&<><section className="simple-profile-stats"><div><span>WORKOUTS</span><strong>{records.length}</strong></div><div><span>WEEKLY RUNNING</span><strong>{weeklyDistance} {distanceUnit}</strong></div><div><span>LONGEST RUN</span><strong>{longestDistance} {distanceUnit}</strong></div><div><span>ACTIVE GOALS</span><strong>{goals.length}</strong></div></section><section className="simple-settings-list"><Link to="/split"><div><strong>Split</strong><span>Training and rest-day structure</span></div><b>›</b></Link><Link to="/history"><div><strong>Activities</strong><span>Calendar and every completed day</span></div><b>›</b></Link><button type="button" onClick={()=>setView('devices')}><div><strong>Recovery &amp; smartwatch</strong><span>Readiness data and device connections</span></div><b>›</b></button><button type="button" onClick={()=>setView('faq')}><div><strong>FAQ</strong><span>How Forge measures, plans and protects your training</span></div><b>›</b></button><Link to="/exercises"><div><strong>Exercise library</strong><span>Browse every movement Forge knows</span></div><b>›</b></Link>{/* A second way in, because the tab strip scrolls and the row that
