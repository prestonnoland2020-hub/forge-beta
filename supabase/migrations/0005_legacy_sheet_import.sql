...
create table if not exists public.data_import_runs (
  ...
  constraint import_status_valid check(status in ('validating','ready','completed','failed'))
);

alter table public.data_import_runs enable row level security;
create policy import_runs_owner_read on public.data_import_runs for select using(owner_id=auth.uid());
