...
begin
  ...
  if not perform_commit then
    ...
    return jsonb_build_object('runId',run_id,'dryRun',true,'source',source_count_payload,'wouldImport',source_count_payload,'warnings',warning_payload);
  end if;
  for day_item in select * from jsonb_array_elements(coalesce(payload->'days','[]'::jsonb)) loop
    if coalesce(day_item->>'date','') !~ '^\d{4}-\d{2}-\d{2}$' then warning_payload:=warning_payload||jsonb_build_array('Skipped day with invalid date: '||coalesce(day_item->>'sourceRow','unknown'));continue;end if;
  ...
  update public.data_import_runs set status='completed',completed_at=now(),warnings=warning_payload,imported_counts=jsonb_build_object('days',imported_days,'topSets',imported_sets,'cardioSessions',imported_cardio,'existingDaysMerged',duplicate_days) where id=run_id;
  return jsonb_build_object('runId',run_id,'dryRun',false,'source',source_count_payload,'imported',jsonb_build_object('days',imported_days,'topSets',imported_sets,'cardioSessions',imported_cardio,'existingDaysMerged',duplicate_days),'warnings',warning_payload);
